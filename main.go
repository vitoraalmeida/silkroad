...
func main() {
	...
	fmt.Println(ps.CreateProduct("O Capital Volume 1", 3, 60.00, 10, true))
	fmt.Println(ps.CreateProduct("O Capital Volume 2", 3, 70.00, 10, true))
	fmt.Println(ps.CreateProduct("O Capital Volume 3", 3, 70.00, 50, true))
	...
	fmt.Println(ps.GetProduct(1))
	err = ps.DecrementProductStock(1, 1)
	...
	ci1 := checkout.CartItem{1, 3, 180.00}
	ci2 := checkout.CartItem{3, 5, 350.00}
	ci3 := checkout.CartItem{4, 4, 120.00}
	...
	fmt.Println("\n---------------------- Sale1's items ----------------- \n")
	saleItems, _ = sis.SearchItemsBySaleID(1)
	...
	ci1 = checkout.CartItem{1, 1, 100.00}
	ci2 = checkout.CartItem{3, 4, 120.00}
	cart = &checkout.Cart{ci1, ci2}
	fmt.Printf("Cart: %+v\n", cart)

	fmt.Println(" checkout 2 ")
	err = chs.Checkout(cart, 2)
	if err != nil {
		fmt.Println(err)
		return
